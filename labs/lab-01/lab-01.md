...
# Part 3. IDE Setup and C++ Tools

In this class you will need multiple tools in order to best succeed, including a bash terminal, a text editor with code highlighting, and a debugging interface. These tools, along with many others, are often packed together into what is called an **Integrated Development Environment**, or IDE. In particular we highly recommend using VSCode for this class, since it is feature complete, fairly easy to use, and best supported by the TAs. 


## 3.1 IDE Options
While [VSCode](https://code.visualstudio.com/Download) is strongly encouraged for this course, some other IDE options are:

- [CLion](https://www.jetbrains.com/clion/) (Free with student id)
- [QT C++](https://www.qt.io/developers/)

These IDEs may require more effort to learn and may seem less user-friendly at times, but they are much more professional and include many of the tools one may actually use in software production. While not necessary for this class, you may find it beneficial to yourself to spend some time learning one. If you would like to use something else, like Vim/Neovim and you are comfortable with the environment, feel free!

## 3.2 Installing GCC  
### Windows Users
If you use Windows 10/11 there is a bit more work to do to get GCC (the compiler we use in CSC211) working. First, we need to install GCC by following the documentation found [here](https://code.visualstudio.com/docs/cpp/config-mingw). Note the video tutorial is helpful, but the directions are sufficient and allow easy copy paste for the install commands!After confirming your MinGW installation, you should be all set!  
### Mac Users
Ensure XCode is up to date, install the official C/C++ plugin on VSCode and you're all set! MacOS ships natively with its own C/C++ Compiler known as Clang/LLVM and uses all of the GCC commands we will cover in this class.  

### Linux Users
All you need to do is install the official C/C++ Plugin in VSCode for syntax highlighting and more advanced features in VSCode. GCC ships natively with all Linux Distros, so you will be all set.
...
If you don't have a terminal already open in your environment, you can create one in the VSCode IDE by clicking File > New Terminal. Please read [this section](https://code.visualstudio.com/docs/terminal/basics) before moving to the next step.
...
Should print out your current file path, if at any point you changed directory with the following command your output will be different.
...
Now that we have changed directory, we can execute the earlier commands to validate our position within the file system. Executing `pwd` from this file should output `/path/to/projects` and executing `ls` should show that there is a file named `hello.cpp` present.
...
Each group will submit a single **text file** named `lab-01.txt` containing all answers to the lab questions and a single `hello.cpp` on [Gradescope](http://gradescope.com).
